...
- `POST /v1/builds/{id}/relationships/betaGroups` answers an unsatisfied build-state precondition with HTTP 422, and the response body does not always name the precondition. `asc builds add-groups` therefore reads `GET /v1/builds/{id}` (`processingState`, `expired`) and, when at least one external group is being added, `GET /v1/builds/{id}/buildBetaDetail` (`externalBuildState`) before the POST, and reports the failing precondition with its remediation command as a validation failure (exit 2) without sending the request. `READY_FOR_BETA_SUBMISSION`, `WAITING_FOR_BETA_REVIEW`, and `IN_BETA_REVIEW` are deliberately not blocking: adding an external group before beta review is the documented way to enable external distribution, so those states only produce an advisory note naming `asc testflight review submit`. State reads are best-effort and bounded: they run under their own budget that reserves at least half of any remaining command deadline for the assignment, and a failed or timed-out read warns on stderr while the assignment still runs. A 422 that survives the preflight renders Apple's own `detail`, `title`, and `code` and keeps exit code 32.
