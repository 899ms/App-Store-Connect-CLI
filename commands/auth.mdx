...
diagnostic and prints the exact, shell-quoted `chmod 600` command that repairs
it. With `--fix-permissions`, asc changes the file to `0600`, reports the change on
stderr, and continues through the normal validation path. A symlinked or
non-regular key path is rejected instead of being changed.
