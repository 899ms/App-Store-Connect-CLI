...
* A missing `app-info/` or `version/<version>/` directory leaves that scope unmanaged: its remote localizations are not read, planned, or deleted. An existing directory, even an empty one, manages every locale in that scope
