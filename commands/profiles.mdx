...
`--stale-only` always reads every page and cannot be combined with `--next`. Its JSON keeps Apple's envelope shape but is a computed view: `data` holds only stale profiles, and `meta` and pagination links are dropped because they describe the unfiltered list. `--include-stale` works with table and markdown output only; with JSON it is a usage error.
